...
void *handle_client(void *arg) {
    ...
    char method[16] = {0}, url[1024] = {0}, protocol[16] = {0};
    ...
    if (strcmp(method, "CONNECT") == 0) {
        ...
        if (colon) {
            ...
            strncpy(host, url, host_len);  // Copy the host part
            host[host_len] = '\0';  // Null-terminate the host

            // Get the port number after the colon
            port = atoi(colon + 1);
            if (port == 0) {
    ...
    // Prepare the request to send to the remote server
    char request[BUFFER_SIZE];
    snprintf(request, sizeof(request), "%s %s %s\r\nHost: %s\r\nConnection: close\r\n", method, path, protocol, host);

    // Check for Content-Length header for POST requests
    if (strcmp(method, "POST") == 0) {
        const char *content_length_header = "Content-Length: ";
        char *content_length_str = strstr(buffer, content_length_header);
        if (content_length_str) {
            // Extract Content-Length
            content_length_str += strlen(content_length_header);
            int content_length = atoi(content_length_str);
            printf("[+] Content-Length: %d\n", content_length);

            // Add Content-Length to the request
    ...
    if (strcmp(method, "POST") == 0) {
        ...
        if (body_start) {
            body_start += 4; // Skip the CRLF separating headers and body
            int body_len = bytes_received - (body_start - buffer);
            send(remote_socket, body_start, body_len, 0);
            printf("[+] Forwarded POST body: %.*s\n", body_len, body_start); // Debugging line
        }
    }

    // Read the response from the remote server and send it back to the client
